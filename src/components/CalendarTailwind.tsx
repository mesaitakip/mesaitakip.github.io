...
interface CalendarTailwindProps {
  currentDate: Date;
...
const CalendarDay = React.memo(({
  ...
  // "Bugün" vurgusu yalnızca o gün, o anda görüntülenen ayın kendi günüyse uygulanır.
  // Aksi halde (ör. Haziran'ı görüntülerken ızgarada taşan Temmuz 1 hücresi) diğer
  // ay taşma günleri gibi soluk/gri görünür — mavi kutu yalnızca kendi ayında çıkar.
  const showAsToday = isTodayDate && isInCurrentMonth;
  ...
  return (
    <div
      ...
      `}
    >
      <span className={`text-sm font-medium ${showAsToday ? 'text-white' : 'dark:text-gray-200'}`}>
        {date.getDate()}
      </span>
      ...
      {holiday && isInCurrentMonth && (
        <div className={`absolute top-0 left-0 text-[0.625rem] sm:text-xs px-1 rounded-br font-bold shadow-sm ${
          isTodayDate 
      ...
      <div className="absolute bottom-0 right-0 flex flex-col items-end pointer-events-none">
        {overtimeEntry && isInCurrentMonth && (
          <div className={`text-[0.5625rem] leading-tight px-1 rounded-tl font-bold shadow-sm ${
            isTodayDate 
              ? 'bg-white/90 text-blue-600' 
              : isHolidayDate
  ...
  return prev.isTodayDate === next.isTodayDate &&
         ...
         prev.overtimeEntries.every((e, i) => {
           ...
           return next_e !== undefined &&
             e.id === next_e.id && 
             calcTotalHours(e) === calcTotalHours(next_e) &&
             e.note === next_e.note
         });
});
...
export const CalendarTailwind: React.FC<CalendarTailwindProps> = React.memo(({ currentDate, onDateChange, onDateClick }) => {
  const {
    isLoaded,
    settings,
    month,
...
CalendarTailwind.displayName = 'CalendarTailwind';
