...
const SectionTitle: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <h3 className="text-[0.625rem] font-bold text-gray-400 uppercase tracking-widest pl-1 mb-1.5 leading-none">
    {children}
  </h3>
);
...
const FeatureItem: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <li className="flex items-center gap-2 py-0.5 px-0.5 transition-colors group">
    ...
    <span className="text-gray-700 dark:text-gray-300 text-[0.625rem] font-bold tracking-tight leading-none">{children}</span>
  </li>
);

export const AboutModalTailwind: React.FC<AboutModalProps> = ({ isOpen, onClose }) => {
          ...
          <div className={`space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-200 ${activeTab === 'about' ? 'block' : 'hidden'}`}>
            ...
            <div className="bg-gradient-to-br from-blue-500 to-indigo-700 rounded-[24px] p-4 text-white shadow-[0_10px_25px_-5px_rgba(59,130,246,0.4),inset_0_2px_4px_rgba(255,255,255,0.3)] border-b-4 border-indigo-900 relative overflow-hidden">
              ...
              <div className="flex justify-between items-center relative z-10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-blue-100 text-[0.5625rem] font-black opacity-90 uppercase tracking-widest mb-0.5 leading-none">v{APP_VERSION}</p>
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="py-1 px-2.5 bg-black/10 rounded-lg backdrop-blur-md border border-white/10 flex flex-col items-center shadow-inner">
                    <span className="text-[0.4375rem] font-black uppercase opacity-60 leading-none mb-0.5">Lisans</span>
                    <span className="text-[0.625rem] font-black leading-tight">GPL-3.0</span>
                  </div>
                  <div className="py-1 px-2.5 bg-black/10 rounded-lg backdrop-blur-md border border-white/10 flex flex-col items-center shadow-inner">
                    <span className="text-[0.4375rem] font-black uppercase opacity-60 leading-none mb-0.5">Durum</span>
                    <span className="text-[0.625rem] font-black leading-tight">Aktif</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <section className="space-y-1.5">
              ...
              <div className="bg-white dark:bg-gray-800/40 rounded-2xl border border-gray-100 dark:border-gray-800 p-3.5 shadow-inner">
                <p className="text-gray-700 dark:text-gray-300 text-[0.6875rem] leading-relaxed font-bold italic opacity-80">
                ...
                <div className="w-full h-full relative z-10 transition-transform duration-500">
                  <Suspense fallback={
                    <div className="flex flex-col items-center justify-center h-full gap-3 p-8 text-center">
                      ...
                      <p className="text-[0.625rem] font-bold text-gray-400 uppercase tracking-widest">Yükleniyor...</p>
                    </div>
                  }>
                    {activeGame === '2048' && <Game2048 win95Enabled={false} />}
                    {activeGame === 'sudoku' && <SudokuGame win95Enabled={false} />}
                    {activeGame === 'wordsearch' && <WordSearchGame win95Enabled={false} />}
                  </Suspense>
                </div>
              </div>
