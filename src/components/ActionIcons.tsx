...
import { Download, Settings, Share2, Trash2, AlertTriangle, MonitorSmartphone } from 'lucide-react';
...
// Ortak kare buton sınıfı (köşeleri biraz kırdım ama daha kare/keskin bir görünüm için 'rounded-lg' yaptım)
  ...
  return (
    <div className={`flex items-center justify-end gap-1.5 ${className}`}>      
      ...
      {/* Win95 Modu — açılabilir retro görünüm */}
      {onToggleWin95 && (
        <button
          onClick={onToggleWin95}
          className={`${buttonClass} bg-gradient-to-br from-teal-500 to-teal-700 border-teal-900 text-white shadow-md`}
          aria-label="Win95 Görünümünü Aç"
          title="Win95 Görünümünü Aç"
        >
          <MonitorSmartphone className="w-4 h-4 relative z-10 group-hover:scale-110 transition-transform" />
        </button>
      )}
      ...
      <button 
        ...
        className="group relative flex items-center justify-center p-2 rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 text-white shadow-[0_4px_10px_-2px_rgba(59,130,246,0.5),inset_0_1px_2px_rgba(255,255,255,0.4)] border-b-2 border-blue-800 active:translate-y-0.5 active:border-b-0 active:scale-95 transition-all overflow-hidden" 
        aria-label="Ayarlar"
      >
        <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-white/20 to-transparent pointer-events-none" />
        <Settings className="w-4 h-4 relative z-10 group-hover:rotate-45 transition-transform duration-300" />
      </button>

      {/* Yedekle */}
      <button 
        onClick={onOpenDataBackup} 
        className="group relative flex items-center justify-center p-2 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-600 text-white shadow-[0_4px_10px_-2px_rgba(16,185,129,0.5),inset_0_1px_2px_rgba(255,255,255,0.4)] border-b-2 border-emerald-800 active:translate-y-0.5 active:border-b-0 active:scale-95 transition-all overflow-hidden" 
        aria-label="Veri Yedekle"
      >
        <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-white/20 to-transparent pointer-events-none" />
        <Download className="w-4 h-4 relative z-10 group-active:translate-y-1 transition-transform" />
      </button>
      ...
      {isShareAvailable && (
        <button 
          ...
          disabled={!canShare}
          className={`group relative flex items-center justify-center p-2 rounded-xl transition-all overflow-hidden ${
            canShare 
        ...
        >
          ...
          <Share2 className="w-4 h-4 relative z-10 group-active:scale-90 transition-transform" />
        </button>
      )}
      ...
      >
        ...
        <Trash2 className="w-4 h-4 relative z-10 group-active:scale-110 transition-transform" />
      </button>
